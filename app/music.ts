...
export const GENRES: Record<Genre, GenreConfig> = {
  ...
  musicBox: { label: "オルゴール", bpm: [52, 68], swing: .5, moods: ["まどろみ", "星明かり", "やすらぎ"], moodEnergy: [1, 1, 1], instruments: { lead: [9], harmony: [0, 46], bass: [43, 42], color: [9, 11] }, layers: { lead: 1, harmony: 1, color: 0 }, melody: { grid: 1, tension: .01, syncopation: .03, rhythms: [[0, 1, 2, 3], [0, 2], [0, 1, 3]] }, energy: [.36, .36, .36, .32], sectionWeights: [1, 1, 1, 1], progressions: [[1, 4, 5, 1]], bass: "pulse", drums: "none", voicing: "open", cadence: [4, 5, 1, 1], form: ["lullaby", "lullaby", "lullaby", "lullaby"], pack: "music-box" },
};
...
export function buildSong(plan: TrackPlan, aiTheme?: NoteEvent[], aiDrums?: NoteEvent[]): Song {
  ...
  const normalized = normalizeNotes(notes, totalBeats);
  if (plan.genre === "musicBox") {
    const lead = normalized.filter(note => note.instrument === 9);
    lead.forEach((note, index) => { note.velocity = 127; note.duration = (lead[index + 1]?.beat ?? totalBeats) - note.beat; });
    normalized.filter(note => plan.instruments.harmony.includes(note.instrument) || note.instrument === plan.instruments.bass).forEach(note => { note.duration = Math.min(totalBeats - note.beat, Math.max(note.duration, 4 - note.beat % 4)); });
  }
  return { plan, notes: normalized, theme, fingerprint: fingerprint(theme), ai: Boolean(aiTheme?.length) };
}
